...
function GoalCard({ title, goal, reward, accent }: { title: string; goal: GoalProgress; reward: number; accent: string }) {
  const theme = useThemeTokens();
  const { t } = useI18n();
  ...
  return (
    <View style={[styles.goalCard, { backgroundColor: theme.surface, borderColor: goal.complete ? accent : theme.gridSep }]}>
      ...
      {/* Nothing under the bar. What used to sit here was `target - done`, which is the
